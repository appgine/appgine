...
function AutoComplete($element, $container, endpoint, onSelect, onUpdate) {
  ...
  this.shouldAutoHilite = function(token, rows) {
  	if ($element.form===undefined) {
  		return true;

  	} else if ($element.form.hasAttribute('action')===false) {
  		return true;
  	}

  	var tokenAscii = appgine.string.toAscii(token).toLowerCase();
  	var titleAscii = appgine.string.toAscii(rows[0] && rows[0]['title']).toLowerCase();

	return tokenAscii===titleAscii;
  }
...
AutoComplete.prototype.renderRows = function(rows)
{
	if (!this.isDisposed()) {
		AutoComplete.category = '';
		var that = this;
		var opt_options = new goog.ui.ac.RenderOptions;
		opt_options.getAutoHilite = function() {
			return that.shouldAutoHilite(that.token_, rows);
		}
