...
import { isSwapping } from '../update'


export default {
	initialState: [],
	progress(state, ident, api) {
		state.push(createListener(ident, api));
	},
	destroy(state) {
...
channel.addListener('auto-submit', 'start', function($form, $element) {
...
channel.addListener('auto-submit', 'submit', function($form, $element) {
	if (autoSubmitForm.indexOf($form)!==-1) {
...
channel.addListener('auto-submit', 'abort', abortAutoSubmit);
channel.addListener('auto-submit', 'destroy', abortAutoSubmit);
...
channel.addListener('app.request', 'start', (endpoint, { $element, requestnum }) => onRequestStart(false, true, endpoint, $element, requestnum));
channel.addListener('app.request', 'response', ({ requestnum }) => onRequestResponse(requestnum));
channel.addListener('app.request', 'end', ({ requestnum }) => onRequestEnd(requestnum));
channel.addListener('app.request', 'abort', ({ requestnum }) => onRequestAbort(requestnum));

channel.addListener('ajax.request', 'start', function(endpoint, { $element, requestnum, isGlobal }) {
	onRequestStart(true, isGlobal, endpoint, $element, requestnum);
});

channel.addListener('ajax.request', 'response', ({ requestnum }) => onRequestResponse(requestnum));
channel.addListener('ajax.request', 'end', ({ requestnum }) => onRequestEnd(requestnum));
channel.addListener('ajax.request', 'abort', ({ requestnum }) => onRequestAbort(requestnum));
